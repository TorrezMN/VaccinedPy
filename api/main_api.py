...
from db_engine.schema import DeviceInfo, Configuration
...
def save_device_info(info: DeviceInfo, db=Depends(db)):
    ...
    if object_in_db:
        raise HTTPException(
            400, detail=crud.error_message('This device info already exists'))
...
def get_device_info(token: str, db=Depends(db)):
    ...
    if info:
        ...
        raise HTTPException(
            404,
            crud.error_message('No device found for token {}'.format(token)))
